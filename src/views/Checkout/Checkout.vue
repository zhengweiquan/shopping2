...
const CheckInfoAPI = async () => {
  ...
  order.value = CartStore.cartList.filter((item) => item.selected === true)
      ...
      <div class="wrapper">
        ...
        <div class="box-body">
          <div class="total">
            <dl>
              <dt>商品件数：</dt>
              <dd>{{ CartStore.sumcheck }}件</dd>
            </dl>
            <dl>
              <dt>商品总价：</dt>
              <dd>¥{{ CartStore.summoney.toFixed(2) }}</dd>
            </dl>
            <dl>
              <dt>运<i></i>费：</dt>
            ...
            <dl>
              <dt>应付总额：</dt>
              <dd class="price">{{ (CartStore.summoney + checkInfo.summary?.postFee).toFixed(2) }}</dd>
            </dl>
          </div>
        </div>
        <!-- 提交订单 -->
  ...
  <el-dialog title="切换收货地址" v-model="showDialog" width="30%" center>
    <div class="addressWrapper">
      <div class="text item" :class="{ active: activeAddress.id === item.id }" v-for="item in checkInfo.userAddresses"
        @click="isclick(item)" :key="item.id">
        <ul>
